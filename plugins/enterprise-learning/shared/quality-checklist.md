...
The `enterprise-code-reviewer` agent evaluates every skill output against this rubric:
...
| Correct Stack | Applications use Spring Boot 4? Libraries/frameworks use plain Java (no Spring)? | Critical |
...
| Insight quality | All ★ Insight blocks have Why + Trade-off + Recommend at minimum? Technology substitutions have strong reasoning? | High |
| Mermaid validity | All diagrams syntactically valid for GitHub rendering? Color palette applied? | High |
| Tutorial structure | All 10 sections (N.1 through N.10) present in every chapter? | High |
| Cross-references | Links between tutorial chapters and technology mapping documents? | Medium |
...
| Jakarta namespace | All code uses `jakarta.*` — never `javax.servlet`, `javax.persistence`? | Critical |
...
| 75 | Significant | Missing ★ Insight on a technology substitution |
| 90 | Critical | Wrong stack for project type (Spring in a library) |
| 100 | Critical | Failing tests, broken compilation, incomplete vertical slice |
...
- Every mention of a technology mapping or deviation detailed in another document MUST include a relative link
- Format: `[Entity Name](./technology-mapping.md#section-anchor)` or `[Deviation](./deviation-report.md#section-anchor)`
- Tutorial chapters reference technology mapping entries for their feature's substitutions
